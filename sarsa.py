...
def train(env, n_episodes=500, max_steps=1000):
    ...
    for episode in range(1, n_episodes+1):
        ...
        for ts in range(max_steps):
            ...
            # get action for next obs using epsilon greedy policy
            next_action = epsilon_greedy(episode, weights, next_obs, order, epsilon)

            # calculate q(next_obs, max action)
            qnext = get_q(weights, next_obs, next_action)

            # calculate current q(obs,action)
            ...
            obs = next_obs
            # make next action current action for next timestep
            action = next_action
